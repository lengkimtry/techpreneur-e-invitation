...
export default function GateAnimation({ onGateOpened, guestName, companyName, logoUrl }: GateAnimationProps) {
  ...
  useEffect(() => {
    ...
    if (btn) {
      gsap.to(btn, {
        boxShadow: "0 0 0 6px rgba(255,216,91,0.35), 0 8px 48px rgba(255,216,91,0.75)",
        duration: 1.2, repeat: -1, yoyo: true, ease: "sine.inOut", delay: 1.2,
      });
    }
      ...
      >
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 mb-4 border border-[#ffd85b]/30 max-w-sm w-full space-y-2.5 text-left flex-shrink-0">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[#f4f1f8] text-sm font-eng">Ministry of Posts and Telecommunications</p>
        ...
        {guestName && (
          <div className="mb-4 w-full max-w-sm flex-shrink-0">
            ...
            <div className="text-center">
              <p className="text-[#c8a0ff]/60 text-xs font-eng uppercase tracking-[0.3em] mb-1">Cordially Invited</p>
              <p className="text-[#ffd85b] font-bold text-2xl sm:text-3xl font-eng">{guestName}</p>
              {companyName && <p className="text-white/70 text-sm font-eng mt-1">{companyName}</p>}
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col items-center gap-1 mb-2 flex-shrink-0">
          <p className="text-[#ffd85b]/70 text-xs font-eng uppercase tracking-widest">Tap to open your invitation</p>
          <svg className="w-5 h-5 text-[#ffd85b]/60 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative w-full max-w-sm flex-shrink-0 flex items-center justify-center">
          ...
          <button
            ...
            style={{ boxShadow: "0 0 0 4px rgba(255,216,91,0.25), 0 6px 32px rgba(255,216,91,0.55)" }}
          >
            {/* Shimmer sweep */}
            <span className="absolute inset-0 rounded-full overflow-hidden pointer-events-none">
              <span
                className="absolute top-0 bottom-0 w-16 bg-white/30 blur-sm -skew-x-12"
